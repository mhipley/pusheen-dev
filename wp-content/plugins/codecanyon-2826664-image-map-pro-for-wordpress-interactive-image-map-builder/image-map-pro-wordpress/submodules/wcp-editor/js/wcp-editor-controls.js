...
;(function ($, window, document, undefined) {
    ...
    $.wcpEditorRegisterControl({
        ...
        getValue: function() {
            ...
            $('#' + this.elementID).find('select').find('option').each(function() {
                var o = { 
                    id: $(this).data('wcp-editor-layers-list-control-option-id'), 
                    title: $(this).data('wcp-editor-layers-list-control-option-title'), 
                    image_url: $(this).data('wcp-editor-layers-list-control-option-image-url'),
                    image_width: $(this).data('wcp-editor-layers-list-control-option-image-width'),
                    image_height: $(this).data('wcp-editor-layers-list-control-option-image-height')
                };
        ...
        setValue: function(v) {
            ...
            for (var i=0; i<v.length; i++) {
                html += '<option data-wcp-editor-layers-list-control-option-id="'+ v[i].id +'" data-wcp-editor-layers-list-control-option-title="'+ v[i].title +'" data-wcp-editor-layers-list-control-option-image-url="'+ v[i].image_url +'" data-wcp-editor-layers-list-control-option-image-width="'+ v[i].image_width +'" data-wcp-editor-layers-list-control-option-image-height="'+ v[i].image_height +'" value="'+ v[i].id +'">'+ v[i].title +'</option>';
            }

            $('#' + this.elementID).find('select').html(html);
        },
        HTML: function() {
            var html = '';

            html += '<div id="'+ this.elementID +'">';
            html += '<select size="10" class="wcp-editor-control-layers-list-select"></select>';
            html += '   <div class="wcp-editor-control-button-group wcp-editor-control-button-group-layers-list">';
            html += '       <div class="wcp-editor-control-button-group-button wcp-editor-control-button-group-button-add"><i class="fa fa-plus" aria-hidden="true"></i></div>';
            html += '       <div class="wcp-editor-control-button-group-button wcp-editor-control-button-group-button-remove"><i class="fa fa-minus" aria-hidden="true"></i></div>';
            html += '       <div class="wcp-editor-control-button-group-button wcp-editor-control-button-group-button-up"><i class="fa fa-arrow-up" aria-hidden="true"></i></div>';
            html += '       <div class="wcp-editor-control-button-group-button wcp-editor-control-button-group-button-down"><i class="fa fa-arrow-down" aria-hidden="true"></i></div>';
            html += '       <div class="wcp-editor-control-button-group-button wcp-editor-control-button-group-button-edit">Edit</div>';
            html += '   </div>';
            html += '   </div>';
        ...
        init: function() {
            ...
            $(document).off('click', '#' + this.elementID + ' .wcp-editor-control-button-group-button-add');
            $(document).on('click', '#' + this.elementID + ' .wcp-editor-control-button-group-button-add', function() {
                self.valueChanged();
                $(document).trigger('event-layers-list-add');
            });
            $(document).off('click', '#' + this.elementID + ' .wcp-editor-control-button-group-button-remove');
            $(document).on('click', '#' + this.elementID + ' .wcp-editor-control-button-group-button-remove', function() {
                // Send event to start the floor delete process
                var v = $('#' + self.elementID + ' select').val();
                $(document).trigger('event-layers-list-remove', v);
            });
            $(document).off('click', '#' + this.elementID + ' .wcp-editor-control-button-group-button-up');
            $(document).on('click', '#' + this.elementID + ' .wcp-editor-control-button-group-button-up', function() {
                var v = $('#' + self.elementID + ' select').val();

                // Remove list item with value "v" from the list
                var list = self.getValue();
                ...
                // Focus the same element
                $('#' + self.elementID + ' select').focus().val(v);

                // Trigger the event
                $(document).trigger('event-layers-list-up');
            });
            $(document).off('click', '#' + this.elementID + ' .wcp-editor-control-button-group-button-down');
            $(document).on('click', '#' + this.elementID + ' .wcp-editor-control-button-group-button-down', function() {
                var v = $('#' + self.elementID + ' select').val();

                // Remove list item with value "v" from the list
                var list = self.getValue();
                for (var i=0; i<list.length; i++) {
                    if (i < list.length - 1 && list[i].id == v) {
                        var tmp = list[i+1];
                        list[i+1] = list[i];
                        list[i] = tmp;
                ...
                // Trigger the event
                $(document).trigger('event-layers-list-down');
            });
            $(document).off('click', '#' + this.elementID + ' .wcp-editor-control-button-group-button-edit');
            $(document).on('click', '#' + this.elementID + ' .wcp-editor-control-button-group-button-edit', function() {
                var v = $('#' + self.elementID + ' select').val();

                self.valueChanged();
                $(document).trigger('event-layers-list-edit', v);
            });
        },
    });
